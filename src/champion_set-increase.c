 ...
 */
/* Time-stamp: <2013/05/20 16:23:18 benavuya.ime.usp.br [benavuya] am> */
...
#define D(w)	((w)->s.dfsum - (w)->s.mate->p.df)
#define L(w)	((w)->s.lsum - (w)->s.mate->p.ell)
...
/*
 * Returns the champion tree set, based on prunning the initial tree one node at a time.
...
Champion_item champion_set_prunning(Tree_node bic_tree) {
    ...
    champion_set->tau = calculate_BIC(bic_tree, 0);
	...
	ITERA(Tree_node, node, word_node->parent, parent) {
	    ...
	    /* 
             * if(!word_node->s.mate->p.ell  || !word_node->s.mate->p.df) { /\* DEBUG *\/
	     * 	DEBS("Alarm:");
	     * 	print_node(word_node->s.mate, "  ",0);
	     * }
             */
	    
	    node->s.lsum += word_node->s.mate->p.ell;
	    node->s.dfsum += word_node->s.mate->p.df;
	    node->s.critical = 0;
	    //printf("%s ", recover_suffix(node)); /* DEBUG */
	    //print_node(node,":::::::",node->depth); /* DEBUG */
	}
    ...
    // when the last tau has only the empty string, we are finished
	...
	// if there are repeat values, this is an incorrect champ tree.
	/* prune at minw */
	minw->s.critical = 1;
 ...
 *     DEB("crit %d\n", bic_tree->s.critical);
 *     
 *     if(!bic_tree->s.critical  && bic_tree->s.dfsum > bic_tree->s.mate->p.df) {
 * 	c = (bic_tree->s.lsum - bic_tree->s.mate->p.ell) /
 * 	    (bic_tree->s.dfsum - bic_tree->s.mate->p.df);
 * 	DEB("minc %f c %f minw %p\n", *minc, c, *node);
 * 	printf("%p %d  %f %f %d %d  %f\n", bic_tree, bic_tree->s.critical,bic_tree->s.lsum, bic_tree->s.mate->p.ell,
 * 	   bic_tree->s.dfsum, bic_tree->s.mate->p.df, c);
...
//    exit(1);
	...
	while (max - min > epsilon) {
	    ...
	    Tau mid_tau = calculate_BIC(bic_tree, mid);
	...
	new_item->tau = calculate_BIC(bic_tree, max);
